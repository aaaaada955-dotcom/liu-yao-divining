    ...
    return ask_qwen([
        {
            ...
            "content": (
                ...
                "语言自然、具体、有温度，约800至1200字。为了便于手机阅读，必须使用 Markdown 排版：开头先单独写一行"
                "**一句话判断：……**，直接给出本次问题的核心结论；之后按内容自拟2至4个短小标题（标题不能是固定模板），"
                "每段最多3句话，不写大段散文。每个标题下只保留一个最重要的判断，并将关键短语加粗；需要列观察点或建议时用项目符号。"
                "不要为了凑格式重复意思。务必完整收尾。"
            ),
        },
        {
    ...
    return ask_qwen([
        {
            ...
            "content": (
                ...
                "使用 Markdown 排版：开头先用 **直接回答：……** 回答追问；之后按内容自拟小标题或项目符号。每段最多3句话，"
                "重点短语加粗，避免一整屏连续散文。"
            ),
        },
        {
